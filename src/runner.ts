...
program
  ...
  .option(
    "--ram <number>",
    "Override the amount of memory in MB to be used by CodeQL extractors. By default, almost all the memory of the machine is used."
  )
  .option(
    "--threads <number>",
...
program
  ...
  .option(
    "--ram <ram>",
    "Amount of memory to use when running queries. Default is to use " +
  ...
  .action(async (cmd: AnalyzeArgs) => {
    ...
    try {
      ...
      const threads = getThreadsFlag(
        cmd.threads || env["CODEQL_THREADS"],
        logger
      );
      const memory = getMemoryFlag(cmd.ram || env["CODEQL_RAM"]);
