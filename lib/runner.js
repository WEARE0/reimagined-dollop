...
program
    ...
    .option("--ram <number>", "Override the amount of memory in MB to be used by CodeQL extractors. By default, almost all the memory of the machine is used.")
    .option("--threads <number>", "The number of threads to be used by CodeQL extractors.")
    .action(async (cmd) => {
...
program
    ...
    .option("--ram <ram>", "Amount of memory to use when running queries. Default is to use " +
    "the same amount of memory as the init command.")
    ...
    "Default is to use the same number of threads as the init command.")
    .option("--temp-dir <dir>", 'Directory to use for temporary files. Default is "./codeql-runner".')
    ...
    try {
        ...
        const env = loadTracerEnvironment(config);
        const threads = (0, util_1.getThreadsFlag)(cmd.threads || env["CODEQL_THREADS"], logger);
        const memory = (0, util_1.getMemoryFlag)(cmd.ram || env["CODEQL_RAM"]);
